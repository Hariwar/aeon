...
from bot import DEFAULT_SPLIT_SIZE, LOGGER, bot, bot_dict, bot_lock, config_dict
from bot.helper.ext_utils.bot_utils import default_button, sync_to_async
...
class TgUploader:
            ...
            for file_ in natsorted(files):
                ...
                try:
                    ...
                    if f_size == 0:
                        corrupted_files += 1
                        LOGGER.error(
                            "%s size is zero, telegram don't upload zero size files",
                            self._up_path,
                        )
                        continue
                    if self._is_cancelled:
                    ...
                    if not self._is_corrupted and (
                        self._listener.isSuperChat or self._leech_log
                    ):
                    ...
                    if isinstance(err, RetryError):
                        LOGGER.info(
                            "Total Attempts: %s",
                            err.last_attempt.attempt_number,
                            exc_info=True,
                        )
                        corrupted_files += 1
        ...
        try:
            async with bot_lock:
                self._client = (
                    ...
                    if (
                        bot_dict["IS_PREMIUM"]
                        and await get_path_size(self._up_path) > DEFAULT_SPLIT_SIZE
                    )
                    or (bot_dict["USERBOT"] and config_dict["USERBOT_LEECH"])
                    else bot
                )
    ...
    async def _user_settings(self):
        self._media_group = self._listener.user_dict.get("media_group", False) or (
            "media_group" not in self._listener.user_dict
        ...
        self._send_pm = self._listener.user_dict.get("enable_pm", False) and (
            self._listener.isSuperChat or self._leech_log
        )
            ...
            for dirpath, _, files in await sync_to_async(walk, self._path):
                for file in files:
                    ...
                    if (
                        ...
                        and (await get_document_type(filepath))[-1]
                    ):
    ...
    def _caption_mode(self, file):
        ...
        if self._user_caption:
            caption = (
                f"""{caption}\n\n{self._user_caption}"""
                if self._user_fnamecap
                else self._user_caption
            )
        return caption
    ...
    async def _final_message(self, ss_image, media_info: bool = False):
        ...
        media_result = (
            await post_media_info(self._up_path, self._size, ss_image)
            if media_info
            else None
        )
        ...
        for mode, link in zip(
            ...
            if link:
                self._buttons.button_link(
                    mode,
                    await sync_to_async(short_url, link, self._listener.user_id),
                    "header",
                )
