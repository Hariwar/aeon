...
class VidEcxecutor(FFProgress):
    ...
    async def _vid_convert(self):
        ...
        for file in file_list:
            ...
            cmd = [
                ...
                "0:v:0",
                "-vf",
                f"scale={self._qual[self.data]}:-2",
                "-map",
                "0:a:?",
                "-map",
                "0:s:?",
        ...
        for file in (file_list := await self._get_files()):
            ...
            if self._metadata:
                ...
                base_dir, self.size = await gather(
                    self._name_base_dir(self.path, "Trim", len(file_list) > 1),
                    get_path_size(self.path),
                )
            self.outfile = ospath.join(base_dir, self.name)
            self._files.append(self.path)
        ...
        for file in file_list:
            ...
            if banner := config_dict["COMPRESS_BANNER"]:
                ...
                cmd.extend(
                    (
                        "-vf",
                        f"subtitles='{sub_file}'{quality},unsharp,eq=contrast=1.07",
                        "-metadata",
                        f"title={banner}",
                        "-metadata:s:v",
                        f"title={banner}",
        ...
        for file in (file_list := await self._get_files()):
            ...
            if self._metadata:
                ...
                base_dir, fsize = await gather(
                    self._name_base_dir(self.path, "Marker", len(file_list) > 1),
                    get_path_size(self.path),
                )
            self.size = fsize + await get_path_size(wmpath)
            self.outfile = ospath.join(base_dir, self.name)
            wmsize, wmposition, popupwm = (
                kwargs.get("wmsize"),
                kwargs.get("wmposition"),
                kwargs.get("popupwm") or "",
            )
            if popupwm:
            ...
            if hardusb and await aiopath.exists(subfile):
                ...
                fontcolour = (
                    f",PrimaryColour=&H{kwargs['fontcolour']}"
                    if kwargs.get("fontcolour")
                    else ""
                )
            ...
            quality = (
                f",scale={self._qual[kwargs['quality']]}:-2"
    ...
    async def _merge_auds(self):
        ...
        if len(self._files) > 1:
            ...
            cmd.extend(
                (
                    f"-disposition:s:a:{audio_track if audio_track == 0 else audio_track + 1}",
                    "default",
                    "-map",
                    "0:s:?",
                    "-c:v",
    ...
    async def _merge_subs(self, **kwargs):
        ...
        for dirpath, _, files in await sync_to_async(walk, self.path):
            ...
            for file in natsorted(files):
                file = ospath.join(dirpath, file)
                is_video, is_sub = (
                    (await get_document_type(file))[0],
                    file.endswith((".ass", ".srt", ".vtt")),
                )
                if is_video:
                    if main_video:
                        continue
