...
async def backup_message(client: Client, message: Message):
    ...
    try:
        ...
        await sendMessage(
            "Your acc need join to source chat!"
            if is_session
            else "You must add me to source chat!",
            message,
        )
        return
    try:
    ...
    await gather(
        deleteMessage(cmsg),
        sendingMessage(text, message, choice(config_dict["IMAGE_COMPLETE"].split())),
    )
